...
### 近期演进 (v0.5)
- [x] **自动裁剪平移优先与安全边界保护**：消除极端边缘构图下的反向切损问题。
- [x] **DOM 增量 XMP 写入与机身评级保护**：原生保护已有调色预设、版权信息与机内评星。
- [x] **四维保存时机与退出守护**：流程完成自动落盘、手动属性修改即时增量防抖保存、显式保存按钮、退出拦截提示。
- [x] **预览视口缩放锁定**：支持跨照片保持放大倍率与平移坐标，便于高倍率盲切比对焦点。
- [x] **参数名称摄影业务化转译**：重构配置参数标签为符合摄影心智的直观术语。

### 中期规划 (v1.0)
- [ ] **预览智能加载高清数据**：常态秒级浏览使用快速预览图；当进行大倍率放大（>150%）或停顿检视时，智能按需异步解码并加载全分辨率 RAW/JPEG 高清数据与矢量 BBox/标签，兼顾极速切图流畅度与纤毫毕现的焦点核验需求。
- [ ] **无监督赛车自动聚类与车队标签标注**：基于赛车主体特征向量与主色调直方图进行无监督聚类，用户单次标注后自动批量赋予同类别赛车车手/车队元数据（IPTC 关键词写入）。
- [ ] **连拍组内同屏多图并排比对视图 (Survey View)**：支持同一连拍组内 2~4 张画面并排像素级联动比对。
- [ ] **选片成果物理导出**：支持一键将高星级选定原片多线程复制至指定交付文件夹。

### 待办储备库 (Backlog)
- [ ] 探索赛车座舱与车手头盔关键点解耦定位（慢门摇摄与高频锐度解耦分析）。
- [ ] 连拍筛选算法引入基于时间轴的局部极值采样（Peak Sampling）。
- [ ] 频域 Tukey 锥度加窗与高 ISO 噪点底噪自适应能量扣除。
- [ ] 跨领域多主体检测与分类解耦（生态鸟类/高速田径体育）。
